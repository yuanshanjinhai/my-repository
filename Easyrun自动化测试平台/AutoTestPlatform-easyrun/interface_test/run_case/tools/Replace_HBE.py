        ...
        if HBE_str[i_index] == "【":
            ...
            if "【时间" in match_string:  # 只有预期返回值里才会有时间数据，而预期返回值是在执行完接口得到实际返回值后才会替换【】
                ...
                if re.findall(r"【a】", match_string) != []:
                    depend_str_list = re.findall('(?<=' + str(expectresponse_key) + '":)[a-zA-Z]+', actual_respone_str)
                    if depend_str_list == []:
                        depend_str = "对应值格式错误！"
                    else:
                        depend_str =  depend_str_list[0]
                ...
                if re.findall(r"【a\d+】", match_string) != []:
                    str_len = match_string[2:-1]
                    depend_str_list = re.findall('(?<=' + str(expectresponse_key) + '":)[a-zA-Z]{' + str_len + '}', actual_respone_str)
                    if depend_str_list == []:
                        depend_str = "对应值格式错误！"
                    else:
                        depend_str = depend_str_list[0]
                if re.findall(r"【d\d+】", match_string) != []:
                    str_len = match_string[2:-1]
                    depend_str_list = re.findall('(?<=' + str(expectresponse_key) + '":)\d{' + str_len + '}', actual_respone_str)
                    if depend_str_list == []:
                        depend_str = "对应值格式错误！"
                    else:
                        depend_str = depend_str_list[0]
                if re.findall(r"【ad\d+】", match_string) != []:
                    str_len = match_string[3:-1]
                    depend_str_list = re.findall('(?<=' + str(expectresponse_key) + '":")\w{' + str_len + '}', actual_respone_str)
                    if depend_str_list == []:
                        depend_str = "对应值格式错误！"
                    else:
                        depend_str = depend_str_list[0]
