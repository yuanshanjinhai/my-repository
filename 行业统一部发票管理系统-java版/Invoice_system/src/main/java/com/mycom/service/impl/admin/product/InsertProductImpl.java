...
@Service
public class InsertProductImpl extends ServiceImpl<SystemProductMapper, SystemProductEntity> implements InsertProductService {
    @Autowired
    ...
    public LinkedHashMap InsertProduct(SystemProductEntity systemProductEntity){
        ...
        if(errorStr != ""){
            ...
            systemProductEntity.setCreat_time(new Date());
            int r = systemProductMapper.insert(systemProductEntity);
            if(r == 0){
                lmp.put("code",0);
                lmp.put("info","faid");
                lmp.put("data","插入失败；");
